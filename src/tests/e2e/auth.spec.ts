...
test.describe('Autenticación', () => {
  ...
  test('debe mostrar error con credenciales vacías', async ({ page }) => {
    ...
    await expect(page.locator('.modal-content p')).toContainText('Por favor, introduce tu email corporativo');
  });
  ...
  test('debe mostrar error de credenciales incorrectas (Mocked)', async ({ page }) => {
    ...
    await expect(page.locator('.error-text')).toContainText('Credenciales incorrectas');
  });
});

...
export const useStockStore = defineStore("stock", () => {
	...
	async function fetchInventory(force = false) {
		...
		if (cachedData.length > 0) {
			...
			console.log(
				"✅ Inventario local (offline) cargado:",
				inventory.value.length,
				"ítems",
			);
		}
	...
	async function syncOutbox() {
		...
		const pendingMovements = await localDb.outbox
			.where("status")
			.equals("PENDING")
			.toArray();
		...
		console.log(
			`🔄 Sincronizando ${pendingMovements.length} movimientos pendientes...`,
		);
		...
		for (const mov of pendingMovements) {
			try {
				...
				if (result && result.status === "ALREADY_PROCESSED") {
					console.warn(
						"⚠️ Movimiento ya había sido procesado previamente:",
						mov.client_mutation_id,
					);
				}
	...
	async function registerMovement(movement: StockMovementInsert) {
		...
		const invItem = inventory.value.find(
			(i) =>
				i.product_id === movement.product_id &&
				i.warehouse_id === movement.warehouse_id,
		);
		if (invItem) {
			invItem.quantity += movement.quantity_change;
			// Persistir el optimismo en local
			await localDb.inventory.update(
				[invItem.product_id, invItem.warehouse_id],
		...
		if (navigator.onLine) {
			...
			console.log(
				"📶 Sin conexión: El movimiento se sincronizará cuando regrese la red.",
			);
		}

...
db.version(6).stores({
	productos:
		"&sku, modelo, categoria, marca, estado, condicion, modeloId, stock, detalle, compatibilidad, createdAt, updatedAt",
...
export async function seedIfEmpty(marcasData, modelosData, categoriasData) {
	...
	const isCorrupt = sampleProducts.some(
		(p) =>
			p.sku.length !== 15 ||
			p.modelo.includes("\ufffd") ||
	...
	if (isCorrupt) {
		console.warn(
			"⚠️ Detectada base de datos incompatible o corrupta. Ejecutando purga total de emergencia...",
		);
		await Promise.all([
			db.productos.clear(),
			db.marcas.clear(),
	...
	if (marcasCount === 0 && marcasData) {
		const entries = Object.entries(marcasData).map(([codigo, nombre]) => ({
			codigo,
			nombre,
		}));
	...
	if (categoriasCount === 0 && categoriasData) {
		const entries = Object.entries(categoriasData).map(([codigo, nombre]) => ({
			codigo,
			nombre,
		}));
...
export async function importInventoryFromCSV(csvText) {
	...
	const productos = rows
		.map((line) => {
			...
			return {
				sku: isOldFormat
					? `${sku.substring(0, 11)}0${sku.substring(11, 14)}`
					: sku,
				...
				modeloId: isOldFormat
					? `0${sku.substring(11, 14)}`
					: sku.substring(11, 15),

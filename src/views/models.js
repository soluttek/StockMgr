...
import { createElement, showCustomModal, showToast } from "../utils/dom.js";
...
export default async function renderModels(container, params) {
	...
	btnBulk.addEventListener("click", async () => {
		const modelsRaw = inputBulk.value
			.split("\n")
			.map((m) => m.trim())
			.filter((m) => m);
		if (modelsRaw.length === 0)
			return showToast("Escribe al menos un modelo", "error");
		...
		showToast(
			`✅ ${added} modelos agregados${skipped ? `. ⚠️ ${skipped} fallaron/duplicados` : ""}`,
	...
	searchInput.addEventListener("input", () =>
		renderModelList(container, marcaCodigo, searchInput.value),
	);
...
async function renderModelList(container, marcaCodigo, filter = "") {
	...
	const allModels = await db.modelos
		.where("marcaCodigo")
		.equals(marcaCodigo)
		.toArray();
	...
	if (filteredModels.length === 0) {
		listEl.innerHTML =
			'<div class="text-center text-muted mt-4">No hay modelos registrados</div>';
		return;
	}
		...
		const item = createElement(
			...
			[
				createElement("span", {
					...
					textContent: model.modeloId,
				}),
				createElement("span", {
					className: "brand-item__name",
					textContent: fullModelName,
				}),
				createElement("span", {
				...
				createElement("button", {
					className: "btn-icon",
					innerHTML:
						'<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>',
					onclick: async (e) => {
						e.stopPropagation();

...
export default async function renderDashboard(container) {
  ...
  const renderCriticalItems = () => {
    ...
    if (activeCategoryFilter) {
      const catName = categoriasData[activeCategoryFilter] || activeCategoryFilter;
  ...
  for (const cat of categoryCountsArr.sort((a, b) => b.count - a.count)) {
    const card = createElement('div', {
      ...
      onclick: () => {
        activeCategoryFilter = (activeCategoryFilter === cat.id) ? null : cat.id;
        renderDashboard(container);
      }
    }, [
      createElement('span', { className: 'category-card__name', textContent: cat.nombre }),
      createElement('span', { className: 'category-card__count', textContent: String(cat.count) })
    ]);

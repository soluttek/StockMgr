...
export default async function renderBrands(container) {
  ...
  `;
    ...
    container.querySelector('#btnAddBrand').addEventListener('click', async () => {
        const codigo = container.querySelector('#inputCodigo').value.toUpperCase().trim();
        ...
        if (existsInJSON || existsInDB) {
            showToast(`⚠️ La marca "${codigo}" ya existe: ${existsInJSON || existsInDB.nombre}`, 'error');
            return;
        }

        const existingName = await db.marcas.where('nombre').equalsIgnoreCase(nombre).first();
        if (existingName) {
            showToast(`⚠️ Ya existe una marca con nombre "${existingName.nombre}" (${existingName.codigo})`, 'error');
            return;
        }
...
async function renderBrandList(container) {
    ...
    for (const brand of dbBrands) {
        if (!marcasData[brand.codigo]) {
            const count = await db.productos.where('marca').equals(brand.codigo).count();
            allBrands.push({ codigo: brand.codigo, nombre: brand.nombre, count, source: 'db' });
        }
    }

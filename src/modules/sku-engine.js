...
export function generateSKU({ categoria, marca, estado, condicion, modeloId }) {
	...
	if (!condicion || condicion.length !== 3)
		throw new Error(`Condición inválida: ${condicion}`);
	if (!modeloId || modeloId.length !== 4)
		throw new Error(`ID Modelo inválido: ${modeloId} (se esperan 4 dígitos)`);

	if (!categoriasData[categoria])
		throw new Error(`Categoría no existe en diccionario: ${categoria}`);
	if (!marcasData[marca])
		throw new Error(`Marca no existe en diccionario: ${marca}`);
	if (!VALID_ESTADOS[estado])
		throw new Error(
			`Estado no válido: ${estado}. Usar: ${Object.keys(VALID_ESTADOS).join(", ")}`,
		);
	if (!VALID_CONDICIONES[condicion])
		throw new Error(
			`Condición no válida: ${condicion}. Usar: ${Object.keys(VALID_CONDICIONES).join(", ")}`,
		);
	...
	if (sku.length !== 15)
		throw new Error(
			`SKU generado con longitud incorrecta: ${sku.length} (esperado: 15)`,
		);
...
export function getEstados() {
	return Object.entries(VALID_ESTADOS).map(([codigo, nombre]) => ({
		codigo,
		nombre,
	}));
}
...
export function getCondiciones() {
	return Object.entries(VALID_CONDICIONES).map(([codigo, nombre]) => ({
		codigo,
		nombre,
	}));
}
...
export function getCategorias() {
	return Object.entries(categoriasData).map(([codigo, nombre]) => ({
		codigo,
		nombre,
	}));
}
...
export function getMarcas() {
	return Object.entries(marcasData).map(([codigo, nombre]) => ({
		codigo,
		nombre,
	}));
}
